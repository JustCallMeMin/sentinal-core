...
func NewTransactionRepository(pool *pgxpool.Pool) *transactionRepository {
	return &transactionRepository{
		BaseRepository: NewBaseRepository[models.Transaction](pool, "transactions"),
	}
}
...
// SaveBulk uses pgx.CopyFrom for maximum performance with large datasets
...
func (r *transactionRepository) ListByTenant(ctx context.Context, tenantID string, limit, offset int) ([]*models.Transaction, error) {
	...
	if err := pgxscan.Select(ctx, r.Pool, &results, query, tenantID, limit, offset); err != nil {
